...
public class ReportGenerator {
    ...
    private static Issues getIssues(HttpGet httpGet) throws Exception {
        ...
        Gson gson = new Gson();
        Issues issues = gson.fromJson(json, com.dto.Issues.class);
        httpClient.close();
        return issues;
    }
        ...
        try {
           ...
           // String filename = "D:\\report\\" + projectName + "-" + date + ".xls";
            ...
            for (int i = 0; i < issues.getIssue().size(); i++) {
             ...
             /*   if(issues.getIssue().get(i).getType().equalsIgnoreCase("CODE_SMELL")) {
                    issues.getIssue().get(i).setType("CODE SMELL");
                    row.createCell(0).setCellValue(issues.getIssue().get(i).getType());
                }if(issues.getIssue().get(i).getType().equhttp://localhost:8080/updateCenter/alsIgnoreCase("VULNERABILITY")) {
                    issues.getIssue().get(i).setType("VULNERABILITIES");
                    row.createCell(0).setCellValue(issues.getIssue().get(i).getType());
                }else{*/
                    row.createCell(0).setCellValue(issues.getIssue().get(i).getType());
               // }
    ...
    private static void createPdf(Issues issues, String projectName)
       ...
       // PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("D:\\report\\" + projectName + "-" + date + ".pdf"));
        ...
        Paragraph paragraph = new Paragraph(" SCANNER REPORT ", FontFactory.getFont(FontFactory.TIMES_ROMAN, 18));
        ...
        preface.add(new Paragraph(" "));
        document.add(preface);
        document.add(createTable("VULNERABILITY", issuesMap.get("VULNERABILITY"), issuesMap.get("FINDBUGS_VULNERABILITY")));
        document.add(createTable("BUGS", issuesMap.get("BUGS"), issuesMap.get("FINDBUGS_BUG")));
        document.add(createTable("CODE_SMELL", issuesMap.get("CODE_SMELL"), issuesMap.get("FINDBUGS_CODE_SMELL")));

        JFreeChart chart = generatePieChart(issuesMap);
        PdfContentByte contentByte = writer.getDirectContent();
        PdfTemplate template = contentByte.createTemplate(300, 350);
        Graphics2D graphics2d = template.createGraphics(300, 330,
    ...
    public static PdfPTable createTable(String type, Map<String, Integer> data,Map<String, Integer> findbugdata) throws DocumentException, IOException {
    ...
    private static JFreeChart generatePieChart(Map<String, Map<String, Integer>> issuesMap) {
        ...
        chart.setPadding(new RectangleInsets(0F, 0F, 100F, 0F));
        return chart;
    }
}
